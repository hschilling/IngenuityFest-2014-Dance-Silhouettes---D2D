    ...
    while (WM_QUIT != msg.message)
    {
        ...
        while (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE))
        {
			...
			if (msg.message == WM_KEYDOWN) {
				if (msg.wParam == 49){
					LoadResourceImage(L"Background", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
				else if (msg.wParam == 50){
					LoadResourceImage(L"Background2", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
				else if (msg.wParam == 51){
					LoadResourceImage(L"Background3", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
				else if (msg.wParam == 52){
					LoadResourceImage(L"Background4", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
				else if (msg.wParam == 53){
					LoadResourceImage(L"Background5", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
				else if (msg.wParam == 54){
					LoadResourceImage(L"Background6", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
				}
			}
                                            ...
                                            const BYTE* pBodyIndexBuffer, int nBodyIndexWidth, int nBodyIndexHeight)
{
	...
	// qqq begin
	//if (nTime - m_nStartTime > 50000000 && !m_image_changed) {
	//	LoadResourceImage(L"Background2", L"Image", cColorWidth, cColorHeight, m_pBackgroundRGBX); // qqq
            ...
            for (int colorIndex = 0; colorIndex < (nColorWidth*nColorHeight); ++colorIndex)
            {
                ...
                if (p.X != -std::numeric_limits<float>::infinity() && p.Y != -std::numeric_limits<float>::infinity())
                {
                    ...
                    if ((depthX >= 0 && depthX < nDepthWidth) && (depthY >= 0 && depthY < nDepthHeight))
                    {
						...
						if (player != 0xff) // if not background
						{
							// set source for copy to the color pixel
							pSrc = m_pColorRGBX + colorIndex;
							switch (player){
							case 0: src = { 0, 255, 0 }; break;
							case 1: src = { 255, 0, 0 }; break;
							case 2: src = { 0, 0, 255 }; break;
							case 3: src = { 255, 255, 0 }; break;
							case 4: src = { 0, 255, 255 }; break;
							case 5: src = { 255, 0, 255 }; break;
							}
                ...
                // write output
				//m_pOutputRGBX[colorIndex] = { 0, 0, 255 }; // QQQ this sets the background but it isn't RGB it is BGR !
				//m_pOutputRGBX[colorIndex] = *pSrc;
				m_pOutputRGBX[colorIndex] = src;
			}
